...
namespace SSTUTools
{
    ...
    public class SSTUCustomFuelTank : PartModule, IPartCostModifier
    {
        ...
        private void updateTankStats()
        {
            ...
            {
                FuelType fuelType = FuelTypes.INSTANCE.getFuelType(currentFuelType);
                if (fuelType != null)
                {
        ...
        private void updatePartResources()
        {
            ...
            FuelType type = FuelTypes.INSTANCE.getFuelType(currentFuelType);
            if (type != null)
            {
    ...
    public class NodeGroup
    {
        ...
        private void updateAttachNode(Part part, float baseHeight, NodeDef nodeDef)
        {
            ...
            {
                SSTUUtils.createAttachNode(part, nodeDef.nodeName, finalPos, nodeDef.nodeOrientation, nodeDef.nodeSize);
            }
            else//move current node, offset attached part/this part
            {
                SSTUUtils.updateAttachNodePosition(part, node, finalPos, nodeDef.nodeOrientation, true);
            }
        }
    }
