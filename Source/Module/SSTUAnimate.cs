...
namespace SSTUTools
{
	...
	public class SSTUAnimate : PartModule
	{
		...
		//	CONFIG FIELD
		// 	if anim status is shown, this will be the status text label
		[KSPField]
		public string animStatusName = "AnimState";

		//	CONFIG FIELD
		// 	should the animation status be shown in the right-click GUI?
		[KSPField]
		...
		[KSPField(isPersistant=true, guiName = "AnimState", guiActive = true, guiActiveEditor=true)]
...
//			print ("SSTUAnimate OnStart");
			...
			else if(animationState==SSTUAnimState.RETRACTING ||animationState==SSTUAnimState.RETRACTED)
			{
		...
		//uses Update() rather than OnUpdate() due to OnUpdate() not firing consistently...(e.g. not in th editor?)
		public void Update()
		{
			...
			{
				print ("SSTUAnimate finished playing forwards");
				setAnimationState (SSTUAnimState.EXTENDED);
				updateGuiLabels();
			}
			else if (animationState == SSTUAnimState.RETRACTING && !isAnimationPlaying ())
			{
				print ("SSTUAnimate finished playing reverse");
				setAnimationState (SSTUAnimState.RETRACTED);
				updateGuiLabels();
			}
		}
